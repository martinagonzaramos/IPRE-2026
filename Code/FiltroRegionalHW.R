## Código modificado parcialmente desde "2.0 HW_process_data.R" en https://github.com/ClimChange-NewbornHealth/HeatWave_PretermBirth_Chile/tree/main/Code 
# Repertorio de José Daniel Conejeros (PUC)

# * Se mantuvieron los comentarios originales en las partes no modificadas del código. 
...
## --- Importar bases de datos temperatura 
tmax <- rio::import("Code/CR2MET_tmax_v2.5_day_COM_TS_1980_2021.csv")
tmin <- rio::import("Code/CR2MET_tmin_v2.5_day_COM_TS_1980_2021.csv")

## --- Ajustar datos de temperatura
...
#Filtro comunal 
# Utilizar códigos comunales correspondientes a paquete "chilemapas"
tmax <- temp %>% 
  left_join(metadata, by="com") %>% 
  filter(com >= 16000 & com < 17000) # Modificar según región
...
com <- chilemapas::codigos_territoriales |> 
  ...
  filter(codigo_region=="16") %>% # Modificar según región
  select(1:2) %>% 
...
writexl::write_xlsx(hw_data, "Output/Heat_Waves_Data_Final[Región].xlsx")
